...
public class LineChartManager {
    ...
    public LineChartManager(LineChart lineChart,float YMiniMum) {
        this.lineChart = lineChart;
        leftYAxis = lineChart.getAxisLeft();
        rightYAxis = lineChart.getAxisRight();
        xAxis = lineChart.getXAxis();

        initChart(lineChart,YMiniMum);
    }

    /**
     * 初始化图表
     */
    private void initChart(LineChart lineChart,float YMiniMum) {
        /***图表设置***/
        //是否展示网格线
...
//        description.setText("需要展示的内容");
        ...
        //保证Y轴从0开始，不然会上移一点
        leftYAxis.setAxisMinimum(YMiniMum);
        rightYAxis.setAxisMinimum(YMiniMum);
